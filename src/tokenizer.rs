...
impl Tokenizer {
    ...
    pub fn for_vocab(vocab_size: usize) -> Self {
        ...
        let pieces = vec![Vec::new(); vocab_size];
        Self {
            vocab_size,
            bos,
            eos,
            pieces,
            by_first: (0..BYTE_VOCAB).map(|_| Vec::new()).collect(),
        }
    }
    ...
    pub fn train(text: &str, vocab_size: usize) -> Self {
        ...
        for word in text.split_whitespace() {
            ...
            for start in 0..bytes.len() {
                for len in 2..=max_piece.min(bytes.len() - start) {
                    *counts.entry(bytes[start..start + len].to_vec()).or_default() += 1;
                }
            }
        }
        ...
        for (offset, (piece, _)) in candidates.into_iter().enumerate() {
            tokenizer.pieces[FIRST_LEARNED_ID + offset] = piece;
        }
    ...
    fn rebuild_index(&mut self) {
        ...
        for ids in &mut self.by_first {
            ids.sort_unstable_by(|&a, &b| {
                self.pieces[b]
                    .len()
                    .cmp(&self.pieces[a].len())
                    .then_with(|| a.cmp(&b))
            });
        }
    }
    ...
    pub fn decode(&self, tokens: &[usize]) -> String {
        ...
        for &token in tokens {
            ...
            } else if token != self.bos && token != self.eos && token < self.pieces.len() {
    ...
    pub fn load<P: AsRef<Path>>(path: P) -> std::io::Result<Self> {
        ...
        if lines.next() != Some("GATOK1") {
            return Err(std::io::Error::new(std::io::ErrorKind::InvalidData, "invalid tokenizer header"));
        }
        ...
        if vocab_size < 258 {
            return Err(std::io::Error::new(std::io::ErrorKind::InvalidData, "tokenizer vocab is too small"));
        }
        let mut tokenizer = Self::for_vocab(vocab_size);
        for line in lines.filter(|line| !line.trim().is_empty()) {
            ...
            if id < FIRST_LEARNED_ID || id >= vocab_size.saturating_sub(2) || hex.len() % 2 != 0 || hex.is_empty() {
...
mod tests {
    ...
    #[test]
    fn learned_tokens_reduce_sequence_length() {
        let tokenizer = Tokenizer::train("hello world hello world hello world", 1024);
        let tokens = tokenizer.encode("hello world hello world");
        assert!(tokens.len() < "hello world hello world".len() + 2);
        assert_eq!(tokenizer.decode(&tokens), "hello world hello world");
    }

    #[test]
    fn tokenizer_round_trip_survives_save_load() {
        let path = env::temp_dir().join(format!("gemma-agent-tokenizer-{}.tok", std::process::id()));
        let tokenizer = Tokenizer::train("hello world hello Rust", 1024);
        tokenizer.save(&path).unwrap();
        let loaded = Tokenizer::load(&path).unwrap();
        let text = "Rust + UTF-8: café\nhello world";
        assert_eq!(loaded.decode(&loaded.encode(text)), text);
        assert_eq!(loaded.vocab_size(), 1024);
        fs::remove_file(path).ok();
    }

    #[test]
    fn special_tokens_are_last_two_ids() {
        let tokenizer = Tokenizer::for_vocab(16_384);
        assert_eq!(tokenizer.bos(), 16_382);
        assert_eq!(tokenizer.eos(), 16_383);
    }
}
