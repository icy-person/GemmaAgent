...
fn train(steps: usize, path: &str, cfg: Config, checkpoint_every: usize, grad_accum: usize, data_path: Option<&str>, targets_per_step: usize, lr: Option<f32>) {
    ...
    assert!(encoded.len() > cfg.context + 1, "training corpus is shorter than configured context");
    ...
    let lr = lr.unwrap_or(if cfg == Config::target() { 0.0005 } else { 0.001 });
...
fn infer(path: &str, prompt: &str, cfg: Config, max_new_tokens: usize, temperature: f32, top_k: usize) {
    ...
    if !std::path::Path::new(path).exists() { eprintln!("checkpoint not found: {path}"); eprintln!("train first"); std::process::exit(2); }
...
fn main() {
    ...
    match args.get(1).map(String::as_str) {
        Some("train") => {
            ...
            let targets_per_step = parse_usize_arg(&args, "--targets-per-step", 32);
            let lr = parse_float_arg(&args, "--lr", if config_from_args(&args) == Config::target() { 0.0005 } else { 0.001 });
            train(steps, path, config_from_args(&args), checkpoint_every, grad_accum, parse_string_arg(&args, "--data"), targets_per_step, Some(lr));
        }
        Some("infer") => {
            ...
            infer(path, prompt, config_from_args(&args), parse_usize_arg(&args, "--tokens", 64), parse_float_arg(&args, "--temperature", 0.0), parse_usize_arg(&args, "--top-k", 0));
        }
...
mod tests {
    ...
    #[test]
    fn greedy_sampling_matches_argmax() { let logits=[0.,-1.,4.,2.]; let mut rng=1; assert_eq!(sample_token(&logits,1e-7,0,&mut rng),2); }
}
