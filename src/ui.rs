...
impl GemmaUi {
    ...
    fn start_job(&mut self) {
        ...
        let mut command = Command::new("cargo"); command.arg("run").arg("--release");
            ...
            match command.current_dir(cwd).spawn() {
                Ok(mut child) => {
                    ...
                    for stream in [stdout.map(BufReader::new), stderr.map(BufReader::new)].into_iter().flatten() {
                        let tx2 = tx.clone(); thread::spawn(move || { for line in stream.lines().flatten() { let _ = tx2.send(line); } });
                    }
                    loop {
        ...
        self.started = Some(Instant::now()); self.notify("Training process started");
    }
    fn nav(&mut self, ui: &mut egui::Ui) {
        ...
        for (page, icon, label) in items {
            let selected = self.page == page; let fill = if selected { self.accent.gamma_multiply(0.18) } else { Color32::TRANSPARENT };
            let button = egui::Button::new(RichText::new(format!("{icon}  {label}")).size(15.0).color(if selected { self.accent } else { ui.visuals().text_color() })).fill(fill).corner_radius(CornerRadius::same(18)).min_size(Vec2::new(190.0, 44.0));
            if ui.add(button).clicked() { self.page = page; } ui.add_space(4.0);
        }
    }
    fn topbar(&mut self, ui: &mut egui::Ui) {
        ui.horizontal(|ui| { ui.label(RichText::new("GemmaAgent").font(FontId::proportional(23.0)).strong()); ui.label(RichText::new("Material You").color(self.accent).size(13.0)); ui.with_layout(Layout::right_to_left(Align::Center), |ui| { let status = if self.job.running() { "● Running" } else { "● Ready" }; ui.colored_label(if self.job.running() { Color32::from_rgb(30,160,90) } else { self.accent }, status); ui.separator(); if ui.button(if self.dark { "☾" } else { "☀" }).clicked() { self.dark = !self.dark; configure_theme(ui.ctx(), self.accent, self.dark, self.compact); } }); });
    }
...
impl eframe::App for GemmaUi {
    fn ui(&mut self, ui: &mut egui::Ui, _frame: &mut eframe::Frame) {
        ...
        egui::Panel::top("top").frame(Frame::new().fill(panel_fill).inner_margin(Margin::symmetric(20,14))).show(ui, |ui| self.topbar(ui));
        let panel_fill = ui.visuals().panel_fill;
        egui::Panel::left("nav").resizable(false).exact_size(220.0).frame(Frame::new().fill(panel_fill).inner_margin(Margin::symmetric(14,20))).show(ui, |ui| {
            ui.label(RichText::new("CONTROL CENTER").size(11.0).color(self.accent)); ui.add_space(14.0); self.nav(ui);
            ui.with_layout(Layout::bottom_up(Align::Center), |ui| { ui.separator(); ui.small("GemmaAgent · Rust"); });
        });
        egui::CentralPanel::default().frame(Frame::new().inner_margin(Margin::symmetric(28,24))).show(ui, |ui| {
            egui::ScrollArea::vertical().auto_shrink([false,false]).show(ui, |ui| match self.page { Page::Dashboard=>self.dashboard(ui), Page::Training=>self.training(ui), Page::Inference=>self.inference(ui), Page::Hardware=>self.hardware(ui), Page::Data=>self.data(ui), Page::Settings=>self.settings(ui), Page::Logs=>self.logs(ui) });
        });
