...
impl GemmaUi {
    ...
    fn poll_logs(&mut self) {
        while let Ok(line) = self.rx.try_recv() {
            if let Some(output) = line.strip_prefix("__PREP_DONE__:") {
                ...
                self.logs.push(line);
                if self.logs.len() > 600 { self.logs.drain(..100); }
            }
        }
    }
    fn project_root() -> Option<PathBuf> {
        ...
        for start in starts.into_iter().flatten() {
            let mut dir = start;
            loop {
                if dir.join("Cargo.toml").is_file() { return Some(dir); }
                if !dir.pop() { break; }
            }
        }
        None
    ...
    fn find_script(name: &str) -> Option<PathBuf> {
        ...
        if let Ok(appdir) = std::env::var("APPDIR") {
            candidates.push(PathBuf::from(appdir).join("usr/share/gemma-agent/scripts").join(name));
        }
        if let Ok(exe) = std::env::current_exe() {
            if let Some(dir) = exe.parent() {
                candidates.push(dir.join(name));
                candidates.push(dir.join("scripts").join(name));
                candidates.push(dir.join("../share/gemma-agent/scripts").join(name));
    ...
    fn find_trainer(name: &str) -> Option<PathBuf> {
        ...
        if let Ok(exe) = std::env::current_exe() {
            if let Some(dir) = exe.parent() {
                candidates.push(dir.join(name)); candidates.push(dir.join("bin").join(name)); candidates.push(dir.join("trainers").join(name));
            }
        }
        if let Some(root) = Self::project_root() {
            candidates.push(root.join("target").join("release").join(name)); candidates.push(root.join("target").join("debug").join(name));
            candidates.push(root.join("bin").join(name)); candidates.push(root.join("trainers").join(name));
        }
        if let Some(path_var) = std::env::var_os("PATH") {
            for dir in std::env::split_paths(&path_var) { let path = dir.join(name); if path.is_file() { return Some(path); } }
        }
        candidates.into_iter().find(|path| path.is_file())
    }
    fn start_prepare_job(&mut self) {
        if self.job.running() { self.notify("Another job is already running"); return; }
        let (script_name, output_dir) = match self.prep_script.as_str() {
            "Real corpus" => ("prepare_real_corpus.py", "data"),
            "Online corpus" => ("prepare_online_corpus.py", "data/online"),
            "Curriculum" => ("prepare_curriculum.py", "data/curriculum"),
            _ => return,
        };
        let script = match Self::find_script(script_name) {
            Some(path) => path,
            None => {
                let message = format!("Preparation script not found: {script_name}");
                self.logs.push(message.clone()); self.notify(message); return;
            }
        };
        ...
        let tx = self.tx.clone(); let child_slot = self.job.child.clone(); let output_dir = output_dir.to_owned();
        thread::spawn(move || {
            let _ = tx.send(format!("Preparing dataset: {script_display}"));
            let _ = tx.send(format!("Output directory: {output_dir}"));
                        ...
                        let done = match child_slot.lock() {
                            Ok(mut slot) => match slot.as_mut() {
                                Some(child) => match child.try_wait() {
                                    Ok(Some(status)) => { let _ = tx.send(format!("Dataset preparation exited: {status}")); if status.success() { let _ = tx.send(format!("__PREP_DONE__:{output_dir}")); } true }
                                    Ok(None) => false,
                                    Err(e) => { let _ = tx.send(format!("Preparation process error: {e}")); true }
                                },
                                None => true,
                            },
                            Err(_) => true,
    ...
    fn start_job(&mut self) {
        ...
        let trainer = match Self::find_trainer(bin) {
            Some(path) => path,
            None => { let message = format!("Trainer not found: {bin}"); self.logs.push(message.clone()); self.notify(message); return; }
        };
        let trainer_display = trainer.display().to_string();
        let working_dir = Self::project_root().or_else(|| std::env::current_dir().ok()).unwrap_or_else(|| PathBuf::from("."));
        let mut command = Command::new(&trainer);
        command.arg("--target").arg("--steps").arg(self.steps.to_string()).arg("--data").arg(&self.data_path).arg("--checkpoint").arg(&self.checkpoint);
        match self.backend {
            Backend::Cpu => {
                command.arg("--grad-accum").arg(self.grad_accum.to_string()).arg("--targets-per-step").arg(self.targets.to_string()).arg("--train-context").arg(self.context.to_string()).arg("--lr").arg(self.lr.to_string());
            ...
            Backend::Gpu => {
                command.arg("--batch-size").arg(self.batch.to_string()).arg("--grad-accum").arg(self.grad_accum.to_string()).arg("--lr").arg(self.lr.to_string()).arg("--checkpoint-every").arg(self.checkpoint_every.to_string()).arg("--eval-every").arg(self.eval_every.to_string()).arg("--gpu").arg(self.gpu.to_string()).arg("--gpu-kind").arg(&self.gpu_kind).arg("--gpu-util").arg(self.gpu_util.to_string());
                if !self.resume.is_empty() { command.arg("--resume").arg(&self.resume); }
            }
        }
        command.current_dir(&working_dir).stdout(Stdio::piped()).stderr(Stdio::piped());
        let tx = self.tx.clone(); let child_slot = self.job.child.clone(); let working_dir_display = working_dir.display().to_string();
        thread::spawn(move || {
            let _ = tx.send(format!("Launching {bin}: {trainer_display}")); let _ = tx.send(format!("Working directory: {working_dir_display}"));
            match command.spawn() {
                Ok(mut child) => {
                    let stdout = child.stdout.take(); let stderr = child.stderr.take();
                    if let Ok(mut slot) = child_slot.lock() { *slot = Some(child); }
                    ...
                    loop {
                        let done = match child_slot.lock() {
                            Ok(mut slot) => match slot.as_mut() {
                                Some(child) => match child.try_wait() {
                                    Ok(Some(status)) => { let _ = tx.send(format!("Process exited: {status}")); true }
                                    Ok(None) => false,
                                    Err(e) => { let _ = tx.send(format!("Process error: {e}")); true }
                                },
                                None => true,
                            },
                            Err(_) => true,
                ...
                Err(e) => { let _ = tx.send(format!("Failed to launch {bin}: {e}")); }
            }
        });
    ...
    fn nav(&mut self, ui: &mut egui::Ui) {
        ...
        for (page, icon, label) in items { let selected = self.page == page; let fill = if selected { self.accent.gamma_multiply(0.18) } else { Color32::TRANSPARENT }; let button = egui::Button::new(RichText::new(format!("{icon}  {label}")).size(15.0).color(if selected { self.accent } else { ui.visuals().text_color() })).fill(fill).corner_radius(CornerRadius::same(18)).min_size(Vec2::new(190.0, 44.0)); if ui.add(button).clicked() { self.page = page; } ui.add_space(4.0); }
    }
    ...
    fn data(&mut self, ui: &mut egui::Ui) {
        ui.heading("Data & checkpoints");
        ui.label("Prepare a corpus from the bundled scripts, then send the generated files directly to the trainer.");
        ui.add_space(12.0);
        Self::card(ui, "Dataset preparation", |ui| {
            ui.horizontal(|ui| { ui.label("Source"); egui::ComboBox::from_id_salt("prep-script").selected_text(&self.prep_script).show_ui(ui, |ui| { for source in ["Real corpus", "Online corpus", "Curriculum"] { ui.selectable_value(&mut self.prep_script, source.to_owned(), source); } }); });
            ui.add_space(8.0);
            match self.prep_script.as_str() {
                "Real corpus" => ui.small("Public-domain books + Tiny Shakespeare → data/train.txt and data/val.txt"),
                "Online corpus" => ui.small("Wikimedia topics → data/online/train.txt and data/online/val.txt"),
                "Curriculum" => ui.small("Mix literature, code, math and reasoning → data/curriculum/*.txt"),
                _ => ui.small("Unknown preparation source"),
            };
            ui.add_space(8.0);
            let disabled = self.job.running();
            ui.add_enabled_ui(!disabled, |ui| { if ui.button("Prepare dataset").clicked() { self.start_prepare_job(); } });
            if disabled { ui.small("A job is already running; wait for it to finish."); }
        });
        ui.add_space(12.0);
        Self::card(ui, "Dataset used by training", |ui| { text(ui, "Train corpus", &mut self.data_path); text(ui, "Validation corpus", &mut self.val_path); ui.small("After preparation, these paths are updated automatically. You can still edit them manually."); });
        ui.add_space(12.0);
        Self::card(ui, "Checkpoint policy", |ui| { text(ui, "Checkpoint path", &mut self.checkpoint); text(ui, "Resume from", &mut self.resume); ui.checkbox(&mut self.auto_save, "Auto-save final checkpoint"); });
    }
