...
impl Value {
    ...
    /// Scale without materializing a full tensor filled with the scalar. The previous
    /// implementation implemented scalar multiplication as `mul(leaf([scalar; N]))`, which
    /// allocated both data and gradient buffers of the full tensor. Attention uses this on
    /// T×T score matrices, so the dedicated op avoids another 8 MiB at T=1024 per invocation.
    pub fn scalar_mul(&self, scalar: f32) -> Self {
        assert!(scalar.is_finite());
        let (r, c) = self.shape();
        let data = self.0.borrow().d.iter().map(|x| x * scalar).collect();
        Self::mk(r, c, data, Op::Scale(self.clone(), scalar))
    }
    ...
    pub fn log(&self) -> Self {
        ...
        let out = {
            let n = self.0.borrow();
            n.d.iter().map(|x| x.max(1e-20).ln()).collect()
        };
        Self::mk(rows, cols, out, Op::Log(self.clone()))
    }
    pub fn neg(&self) -> Self {
        let (rows, cols) = self.shape();
        let out = {
            let n = self.0.borrow();
            n.d.iter().map(|x| -x).collect()
        };
        Self::mk(rows, cols, out, Op::Neg(self.clone()))
    }
    pub fn silu(&self) -> Self {
        ...
        Self::mk(rows, cols, out, Op::Silu(self.clone()))
    }
        ...
        for r in &refs {
            ...
            for i in 0..rows {
                out[i * cols + offset..i * cols + offset + width]
                    .copy_from_slice(&r.d[i * width..(i + 1) * width]);
            }
            offset += width;
        }
        drop(refs);
    ...
    pub fn slice_cols(&self, start: usize, len: usize) -> Self {
        ...
        let out = {
            ...
            for i in 0..rows {
                out.extend_from_slice(&n.d[i * cols + start..i * cols + start + len]);
            }
            out
        };
    ...
    pub fn gather(&self, index: usize) -> Self {
        ...
        assert!(index < cols);
        let value = self.0.borrow().d[index];
        Self::mk(1, 1, vec![value], Op::Gather(self.clone(), index))
    }
    pub fn row(&self, row: usize) -> Self {
        let (rows, cols) = self.shape();
        assert!(row < rows);
        let out = self.0.borrow().d[row * cols..(row + 1) * cols].to_vec();
...
fn topo(value: &Value, seen: &mut HashSet<usize>, order: &mut Vec<Value>) {
    ...
    match value.0.borrow().op.clone() {
        ...
        Op::Scale(a, _) => topo(&a, seen, order),
        Op::Transpose(a)
        | Op::Log(a)
        | Op::Neg(a)
        | Op::Softmax(a)
...
fn back(value: &Value) {
    ...
    match value.0.borrow().op.clone() {
        ...
        Op::Scale(a, scalar) => {
            accumulate(&a, &grad.iter().map(|g| g * scalar).collect::<Vec<_>>())
        }
        Op::Mul(a, b) => {
            let (ga, gb) = {
                let na = a.0.borrow();
        ...
        Op::RmsNorm(a, eps) => {
            ...
            let ga = {
                ...
                for r in 0..rows {
                    let x_row = &na.d[r * cols..(r + 1) * cols];
                    let g_row = &grad[r * cols..(r + 1) * cols];
                    let mean_sq = x_row.iter().map(|v| v * v).sum::<f32>() / n;
                    let rr = (mean_sq + eps).sqrt();
                    let inv = 1.0 / rr;
                    let dot = g_row.iter().zip(x_row).map(|(g, x)| g * x).sum::<f32>();
                    let coeff = dot / (n * rr * rr * rr);
                    let ga_row = &mut ga[r * cols..(r + 1) * cols];
                    for j in 0..cols {
                        ga_row[j] = g_row[j] * inv - x_row[j] * coeff;
                    }
                }
                ga
            };
        ...
        Op::Softmax(a) => {
            ...
            let ga = {
                let ny = value.0.borrow();
                let mut ga = vec![0.0f32; rows * cols];
                for r in 0..rows {
                    let y_row = &ny.d[r * cols..(r + 1) * cols];
                    let g_row = &grad[r * cols..(r + 1) * cols];
        ...
        Op::ConcatCols(xs) => {
            ...
            for x in xs {
                ...
                for row in 0..rows {
                    gx[row * width..(row + 1) * width].copy_from_slice(
                        &grad[row * cols + offset..row * cols + offset + width],
                    );
                }
                accumulate(&x, &gx);
                offset += width;
        ...
        Op::Gather(a, index) => {
            let mut ga = vec![0.0; a.shape().1];
            ga[index] = grad[0];
            accumulate(&a, &ga);
        }
        Op::RowGather(a, row) => {
            let cols = a.shape().1;
            let mut ga = vec![0.0; a.shape().0 * cols];
            ga[row * cols..(row + 1) * cols].copy_from_slice(&grad[..cols]);
            accumulate(&a, &ga);
        }
    }
}

fn matmul_raw(a: &[f32], b: &[f32], ar: usize, ac: usize, bc: usize) -> Vec<f32> {
    let mut out = vec![0.0; ar * bc];
    if ar * ac * bc >= 200_000 {
        let chunks = (0..ar).collect::<Vec<_>>();
        std::thread::scope(|scope| {
            let workers = std::thread::available_parallelism()
                .map(|n| n.get())
                .unwrap_or(1)
                .min(ar.max(1));
            let chunk_size = (ar + workers - 1) / workers;
                ...
                scope.spawn(move || {
                    for &i in chunk {
                        for k in 0..ac {
                            let av = a[i * ac + k];
                            for j in 0..bc {
                                unsafe { *out_ptr.add(i * bc + j) += av * b[k * bc + j]; }
                            }
                        }
                    }
                });
