...
fn xorshift64(state: &mut u64) -> u64 {
    let mut x = *state; x ^= x << 13; x ^= x >> 7; x ^= x << 17; *state = x; x
}
...
fn sample(values: &[f32], temperature: f32, top_k: usize, rng: &mut u64) -> usize {
    ...
    let mut acc = 0.0;
    for (i, w) in ids.iter().zip(weights) { acc += w; if acc >= threshold { return *i; } }
    *ids.last().unwrap()
}
...
fn main() {
    ...
    let model = amd::AmdModelConfig::new(cfg)
        .init(&device)
        .load_file(&checkpoint, &burn::record::BinFileRecorder::<burn::record::FullPrecisionSettings>::default(), &device)
        .unwrap_or_else(|e| panic!("failed to load checkpoint '{checkpoint}': {e}"));
    let target = cfg.vocab == config::Config::target().vocab;
    let mut tokens = if target { amd_tokenizer::AmdTokenizer::load(&tokenizer_path).unwrap_or_else(|e| panic!("failed to load tokenizer '{tokenizer_path}': {e}")).encode(&prompt) } else { tokenizer::Tokenizer::new().encode(&prompt) };
    let eos = if target { amd_tokenizer::EOS } else { tokenizer::EOS };
    if tokens.last().copied() == Some(eos) { tokens.pop(); }
    ...
    for _ in 0..max_new {
        ...
        if next == eos || tokens.len() >= cfg.context { break; }
        logits = amd::inference_step(&model, next, &mut cache, &device);
    }
    if target { let tok = amd_tokenizer::AmdTokenizer::load(&tokenizer_path).unwrap_or_else(|e| panic!("failed to reload tokenizer: {e}")); println!("{}", tok.decode(&tokens)); }
...
fn main() {
    ...
    eprintln!("For Android arm64: cargo ndk -t arm64-v8a build --release --no-default-features --features android-vulkan --bin android-infer");
}
